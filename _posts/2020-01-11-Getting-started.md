...
The instructions are stored in the form of bits... The _encoding_ of an instruction (What sequence of bits corresponds to what instruction) is specified by the *ISA* (of course). The instruction pointer is simply a *register* that points to the current instruction, ie, it has the address value of the current instruction stored in it. (Well, during the instruction execution, it gets updated to point to the next instruction, ... So, in a way it points to the next instruction when it's close to the end, and the current instruction at the beginning. This is a subtle **microarchitectural** detail that depends on the guy who made your PC) 
