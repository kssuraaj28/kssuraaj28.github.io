...
The instructions are stored in the form of bits... The _encoding_ of an instruction (What sequence of bits corresponds to what instruction) is specified by the *ISA* (of course). The instruction pointer is simply a *register* that points to the current instruction, ie, it has the address value of the current instruction stored in it. (Well, the x86 ISA says that it must be shown that during an instruction, the address of the next instruction is stored in the PC. This is a subtle detail that you might want to keep in mind if you want to be a *true binary god*) All that I can tell you is: Any trouble? Look into the 2000-3000 page ISA book, become an old man, and then come back to me ;)
...
### Conclusion  
Phew, that was a lot! In the next blog, I talk about how life is simple, and so are assemblers. I might also talk about the chicken and egg problem of needing a working OS to write an OS.
